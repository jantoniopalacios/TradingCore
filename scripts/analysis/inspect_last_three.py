...
with app.app_context():
    # Encontrar las Ãºltimas 3 tandas (id_estrategia por usuario)
    q = db.session.query(
    ...
    if not tandas:
        ...
        for usuario_id, id_estrategia, max_date in tandas:
            print('='*60)
            print(f'Tanda: {id_estrategia} | Usuario ID: {usuario_id} | Fecha Ãºltima: {max_date}')
            ...
            for r in resultados:
                print('-'*40)
                print(f'ID resultado: {r.id} | SÃ­mbolo: {r.symbol} | Fecha ejecuciÃ³n: {r.fecha_ejecucion}')
